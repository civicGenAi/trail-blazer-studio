    ...
    <>
      <section className="grain-dark pb-14 pt-12">
        <div className="container-editorial">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-cream/55">
            ...
            <ChevronRight className="h-3 w-3" aria-hidden />
            <span className="text-cream/85">{post.category}</span>
          </nav>
          <h1 className="mt-6 max-w-4xl text-3xl text-cream sm:text-4xl lg:text-6xl">
            {post.title}
          </h1>
          <p className="field-note mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-cream/55">
      ...
      <article className="py-16 md:py-20">
        <div className="container-editorial grid gap-12 lg:grid-cols-[0.72fr_0.28fr]">
          <div className="max-w-2xl">
            <p className="border-l-2 border-gold pl-5 text-xl leading-relaxed">{post.excerpt}</p>
          ...
          <ul className="mt-8 grid gap-px bg-cream/10 md:grid-cols-3">
            {more.map((p) => (
              <li key={p.slug} className="bg-savanna">
                ...
                >
                  <span className="field-note text-gold">{p.category}</span>
                  <span className="mt-2 font-display text-xl leading-tight text-cream">
                    {p.title}
                  </span>
                  <span className="mt-3 text-xs text-cream/55">{formatPostDate(p.date)}</span>
                </Link>
              </li>
            ))}
          </ul>

...
export function DestinationMap({ onSelect }: { onSelect?: (slug: string) => void }) {
  ...
  return (
    <div>
      <div className="relative aspect-[4/3] w-full border border-mist/15 bg-savanna-deep sm:aspect-[16/10]">
        ...
        >
          <span className="block h-2 w-2 rounded-full bg-mist" aria-hidden />
          <span className="field-note absolute left-1/2 top-full mt-1.5 hidden -translate-x-1/2 whitespace-nowrap text-[0.55rem] text-mist/60 sm:block">
            {originPin.name}
          </span>
        </span>
            ...
            >
              <span
                className={`block h-3 w-3 rotate-45 border transition-colors duration-300 ${
                  active === d.slug
                    ? "border-summit bg-summit"
                    : "border-mist/70 bg-savanna group-hover:border-summit"
                }`}
              />
              <span
                className={`field-note absolute top-full mt-1 hidden whitespace-nowrap text-[0.55rem] transition-colors duration-300 sm:block ${
                  active === d.slug ? "text-summit" : "text-mist/60"
                } ${
                  edge === "right"
                    ? "right-1"
                    : edge === "left"
        ...
        {airports.map((a) => (
          <span
            ...
            title={a.name}
          >
            <Plane className="h-3.5 w-3.5 text-mist/50" aria-hidden />
            <span className="field-note absolute left-1/2 top-full mt-1 hidden -translate-x-1/2 text-[0.5rem] text-mist/40 sm:block">
              {a.short}
            </span>
      ...
      <div
        className="mt-px flex min-h-[4.75rem] items-start gap-2.5 border border-t-0 border-mist/15 bg-savanna p-4"
        aria-live="polite"
      >
        {shown ? (
          <>
            <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0 text-summit" aria-hidden />
            <span>
              <span className="field-note block text-summit">{shown.region}</span>
              <span className="mt-1.5 block text-sm text-mist">{shown.name}</span>
              <span className="mt-1 block text-xs text-mist/60">Best time · {shown.best_time}</span>
            </span>
          </>
        ) : (
          <>
